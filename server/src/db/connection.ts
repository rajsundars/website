...
export async function logAudit(userId: string | null, userName: string | null, action: string, details: string) {
...
export async function initializeDatabase() {
  ...
  if (payrollCount === 0) {
    ...
    for (const s of staffSalaries) {
      const uId = s[0] as string;
      const base = s[1] as number;
      const inc = s[2] as number;
