...
router.post("/", verifyToken, requireRole(["super_admin"]), validateBody([
  ...
  try {
    ...
    await logAudit(
      ...
      "employee_create",
      `User ${req.user?.name || "System"} registered new employee: ${name} (${role}) assigned to ${branchId}.`
    );
...
router.put("/:id", verifyToken, requireRole(["super_admin"]), validateBody([
  ...
  try {
    ...
    await logAudit(
      ...
      req.user?.name || "System",
      "employee_update",
      `User ${req.user?.name || "System"} updated employee record: ${name} (${id}).`
    );
...
router.delete("/:id", verifyToken, requireRole(["super_admin"]), async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    await logAudit(
      ...
      req.user?.name || "System",
      "employee_delete",
      `User ${req.user?.name || "System"} deleted employee: ${empName} (${id}).`
    );
...
router.post("/shifts", verifyToken, requireRole(["super_admin", "branch_manager"]), async (req: AuthenticatedRequest, res: Response) => {
...
router.post("/attendance/clock-in", verifyToken, async (req: AuthenticatedRequest, res: Response) => {
...
router.post("/attendance/clock-out", verifyToken, async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
...
router.post("/payroll/process", verifyToken, requireRole(["super_admin"]), async (req: AuthenticatedRequest, res: Response) => {
