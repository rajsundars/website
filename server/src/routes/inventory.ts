...
router.post("/adjust", verifyToken, async (req: AuthenticatedRequest, res: Response) => {
...
router.post("/transfer", verifyToken, async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    ...
    const fromBranchName = branches.find((b: any) => b.id === fromBranchId)?.name || "Source Cellar";
    const toBranchName = branches.find((b: any) => b.id === toBranchId)?.name || "Destination Cellar";
