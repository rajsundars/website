...
router.get("/", async (req, res) => {
    ...
    try {
        ...
        const evaluated = customers.map((c) => {
...
router.post("/", verifyToken, requireRole(["super_admin", "event_coordinator"]), validateBody([
    ...
    `, [customerId, name, email, phone, address || "", points, segment, regDate]);
        await logAudit(req.user?.id || "unknown", req.user?.name || "System", "customer_create", `User ${req.user?.name || "System"} registered new customer profile: ${name} (Email: ${email}).`);
...
router.put("/:id", verifyToken, requireRole(["super_admin", "event_coordinator"]), validateBody([
    ...
    `, [name, email, phone, address || "", points, segment, id]);
        await logAudit(req.user?.id || "unknown", req.user?.name || "System", "customer_update", `User ${req.user?.name || "System"} updated customer profile: ${name} (${id}).`);
...
router.delete("/:id", verifyToken, requireRole(["super_admin"]), async (req, res) => {
    ...
    try {
        ...
        await logAudit(req.user?.id || "unknown", req.user?.name || "System", "customer_delete", `User ${req.user?.name || "System"} deleted customer profile: ${customerName} (${id}).`);
...
router.post("/:id/notify", verifyToken, requireRole(["super_admin", "event_coordinator"]), async (req, res) => {
    const { id } = req.params;
