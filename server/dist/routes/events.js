...
router.patch("/:id/status", verifyToken, requireRole(["super_admin", "event_coordinator"]), async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        await logAudit(req.user?.id || "unknown", req.user?.name || "System", "event_status_update", `User ${req.user?.name || "System"} updated event "${event.customer_name}" status from ${oldStatus} to ${newStatus}.`);
...
router.post("/:id/wines", verifyToken, requireRole(["super_admin", "event_coordinator"]), async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        await logAudit(req.user?.id || "unknown", req.user?.name || "System", "event_wines_update", `User ${req.user?.name || "System"} updated wine allocations for event "${event.customer_name}" (${id}).`);
...
router.post("/:id/staff", verifyToken, requireRole(["super_admin", "event_coordinator"]), async (req, res) => {
    const { id } = req.params;
    ...
    try {
      ...
      `, [sId, event.date, id]);
        ...
        await logAudit(req.user?.id || "unknown", req.user?.name || "System", "event_staffing_update", `User ${req.user?.name || "System"} updated staffing roster for event "${event.customer_name}" (${id}).`);
